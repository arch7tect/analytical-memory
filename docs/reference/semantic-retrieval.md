...
ignored by Git. External embeddings are public-only; private search documents
are excluded before any provider call.
...
provider contract, so changing the model, dimensions, preprocessing, or scope
creates a new profile and therefore new embedding records.

Rebuild is explicit because it performs billable remote calls. It sends only
public eligible text. Inputs are normalized to NFC,
...
Search applies exact namespace and node-type filters before ranking
